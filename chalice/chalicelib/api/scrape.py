...
import chalicelib.utils.utils as res
from chalicelib.utils.utils import headers
...
class Vlr:
    ...
    def vlr_recent(self):
        ...
        for item in html.css('a.wf-module-item'):
            ...
            desc = item.css_first("div").css_first(
                "div:nth-child(2)").text().strip()
            ...
            title = item.css_first(
                "div:nth-child(1)").text().strip().split('\n')[0]
            title = title.replace('\t', '')
    ...
    def vlr_rankings(region):
        ...
        for item in html.css("div.rank-item"):
            ...
            # get logo png url
            logo = item.css_first(
                "a.rank-item-team").css_first("img").attributes['src']
            ...
            last_played_team_logo = item.css_first(
                "a.rank-item-last").css_first("img").attributes['src']
